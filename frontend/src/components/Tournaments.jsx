...
function Tournaments() {
  ...
  const fetchMatches = async (search = '') => {
    try {
      ...
      setMatchError('')
      const query = search ? `?jugador=${encodeURIComponent(search)}` : ''
      const res = await fetch(`${API_URL}/api/partidos${query}`)
      const body = await res.json()

      if (body.ok && body.datos) {
        setMatches(body.datos)
      } else {
        setMatchError(body.mensaje || 'Error cargando partidos')
      }
    } catch (err) {
  ...
  const startEdit = (tournament) => {
    ...
    setEditingData({
      ...
      fecha_inicio: formatDate(tournament.fecha_inicio),
      fecha_fin: formatDate(tournament.fecha_fin)
    })
    setError('')
    setSuccess('')
  }
      ...
      <div className="row mt-5">
        <div className="col-12">
          <div className="card bg-dark border-secondary p-4">
            <h3 className="text-info mb-3" style={subtitleStyle}>Partidos en torneos</h3>

            <form className="row g-2 align-items-end mb-4" onSubmit={handleMatchSearch}>
              <div className="col-md-8">
                <label className="form-label text-white" style={labelStyle}>Buscar por jugador</label>
                <input
                  type="text"
                  className="form-control"
                  style={inputStyle}
            ...
            {matchesLoading ? (
              ...
              <div className="alert alert-info">No se encontraron partidos.</div>
            ) : (
              <div className="row g-3">
                {matches.map((match) => {
                  const jugador1Name = `${match.jugador1?.nombre || 'Jugador 1'} ${match.jugador1?.apellido || ''}`.trim()
                  const jugador2Name = `${match.jugador2?.nombre || 'Jugador 2'} ${match.jugador2?.apellido || ''}`.trim()
                  ...
                  const resultado = match.resultado_jugador1 !== null && match.resultado_jugador2 !== null
                    ...
                    : 'Pendiente'
                  const fechaPartido = match.fecha_partido ? new Date(match.fecha_partido).toLocaleDateString('es-ES') : 'Fecha no definida'

                  return (
                    <div key={match.id} className="col-md-6">
                      <div className="card bg-secondary bg-opacity-10 border-light">
                        <div className="card-body">
                          <h5 className="card-title text-white">{torneoNombre}</h5>
                          <p className="card-text text-white-50 mb-1">{fechaPartido} · {match.estado || 'sin estado'}</p>
                          <p className="card-text text-white mb-1"><strong>{jugador1Name}</strong> vs <strong>{jugador2Name}</strong></p>
                          <p className="card-text text-white mb-0">Resultado: {resultado}</p>
                        </div>
                      </div>
                    </div>
                  )
